...
struct hash<math::Vertex>
{
    ...
    {
        return std::hash<float>()(vertex.X) ^ (std::hash<float>()(vertex.Y) << 1);
    }
};
} // namespace std
...
public:
    ...
    virtual void begin(duDebugDrawPrimitives prim, float size = 1.f) override;
    ...
    virtual void vertex(const float x, const float y, const float z,
                        unsigned int color, const float u, const float v) override;
